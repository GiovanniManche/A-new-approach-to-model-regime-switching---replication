function [nloglik, filtered_probs_final] = likelihood_function(params, y, k, is_vol_model)
    ...
    % Computes the likelihood function, using for that the Chang, Chain and 
    ...
    for t = 1:eff_T
        ...
        for j = 1:N_configs
            ...
            % We take the probability mass from the previous configuration j (prob_state(j))
            % and distribute it to the two possible future configurations at time t.
            %
            % - omega     : Probability of transitioning to Regime 0 (Low)
            % - (1-omega) : Probability of transitioning to Regime 1 (High)
            %
            % Note: We use '+=' because multiple previous configurations can collapse 
            % into the same future configuration (due to the shifting memory window).
