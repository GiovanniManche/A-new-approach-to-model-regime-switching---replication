...
% Load the raw data generated by the previous script
% Assurez-vous que ce fichier est bien dans le dossier courant ou ajustez le chemin
...
%% 1. FIGURE: DENSITY OF RHO ESTIMATOR (Replicating Fig 3)
% We want to show the distribution of Rho_hat for True Rho = 0, -0.5, -1
models = {'Mean', 'Vol'};
...
for m = 1:length(models)
    ...
    for s = 1:length(scenarios)
        ...
        if isfield(MC_Data, field)
            ...
            % Plot Vertical Line for True Value
...
%% 2. FIGURE: BIAS AND RMSE (Replicating Fig 4 style)
% Showing Bias and RMSE as a function of True Rho
for m = 1:length(models)
    mod = models{m};
...
% Important to validate the test statistic
for m = 1:length(models)
    mod = models{m};
    field = sprintf('%s_Rho_0', mod); % We look at H0 case
